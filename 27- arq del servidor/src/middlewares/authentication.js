...
passport.use('github-login', new GithubStrategy({
  ...
  // console.log(profile)
  const usuario = await usuariosService.findOneUser({ email: profile.username })
  ...
  try {
    const registrado = await usuariosService.registrar({
...
passport.use('local-register', new LocalStrategy({
  ...
  async (req, _u, _p, done) => {
    try {
      const datosUsuario = await usuariosService.registrar(req.body)
...
passport.use('local-login', new LocalStrategy({
  ...
  try {
    const datosUsuario = await usuariosService.autenticar(email, password)
    done(null, datosUsuario)
  } catch (error) {
